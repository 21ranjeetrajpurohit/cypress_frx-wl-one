...
import users from "../fixtures/RKCostProjections.json"
...
describe('Fiduciary Testing', () => {
    ...
    for (let user in users) {
        ...
        let add_rk_cost_projections = users[user].add_rk_cost_projections;
        let delete_rk_cost_projections = users[user].delete_rk_cost_projections;
        ...
        it('Check Total Cost Comparison Page Access', () => {
            ...
            cy.get('@login').then((xhr) => {
                ...
                cy.get('@access').then((xhr) => {
                    page_access = xhr.response.body.Access.rk_cost_projections;
        ...
        it('check Total Cost Comparison Page all CRUD visible', () => {
            ...
            cy.visit('https://frx-wl-one.slashash.dev/fee-rx/rk-cost-projections/all')
        ...
        if (access.view == true && access.add == true) {
            it('RK Cost Projections Add and Get Success Status', () => {
                //login
                cy.loginByForm(email, password)
                cy.visit('https://frx-wl-one.slashash.dev/fee-rx/rk-cost-projections/all');
                //Verify element on page
                ...
                cy.fixture('ReportRKCostProjections.json').then((data) => {
                    cy.intercept('POST', '**/api/reports/fee-rx/generate-rk-cost-projections').as('generate_report');
                    ...
                    cy.get('@generate_report').then((xhr) => {
                        expect(xhr.response.statusCode).to.eq(add_rk_cost_projections.success);
                    })
                })
            })
        }
        ...
        if (access.view == true && access.add == false) {
            it('RK Cost Projections Add and Get unauthorized Status', () => {
                //login
                cy.loginByForm(email, password)
                cy.visit('https://frx-wl-one.slashash.dev/fee-rx/rk-cost-projections/all');
                //Verify element on page
                ...
                cy.fixture('ReportRKCostProjections.json').then((data) => {
                    ...
                    cy.wait('@generate_report', { requestTimeout: 20000 });
                    cy.get('@generate_report').then((xhr) => {
                        expect(xhr.response.statusCode).to.eq(add_rk_cost_projections.unauthorized);
                    })
                })
            })
        }
        ...
        if (access.view == true && access.export == false) {
            it('Export Report', () => {
                //login
                cy.loginByForm(email, password)
                cy.intercept('GET', '**/api/reports/list?limit=25&report_id=4&report_rule=rk%20cost%20projections&f_ps_id=&f_report_date=&report_type_id=4&offset=0').as('report_list');
                cy.visit('https://frx-wl-one.slashash.dev/fee-rx/rk-cost-projections/all');
                //Verify element on page
                cy.get('.mb-0.me-auto').should('have.text', page_name);
                //Report list
                    ...
                    cy.request('GET', 'https://frx-wl-one.slashash.dev/api/testing/get-token?report_id=' + xhr.response.body.qReports.data[0].report_id).then(
                        (response) => {
                            ...
                            cy.fixture('ReportRKCostProjections.json').then((data) => {
                                ...
                                cy.get(':nth-child(3) > :nth-child(6)').should('contain', data.current_per_participant);
                                ...
                                cy.get(':nth-child(3) > :nth-child(6)').should('contain', data.proposed_per_participant);
                            })
                        }
                    )
                })
        ...
        if (access.view == true && access.delete == true) {
            it('RK Cost Projections delete and Get Success Status', () => {
                //login
                cy.loginByForm(email, password)
                cy.visit('https://frx-wl-one.slashash.dev/fee-rx/rk-cost-projections/all');
                //Verify element on page
                ...
                cy.fixture('ReportRKCostProjections.json').then((data) => {
                    ...
                    cy.get('@delete_report').then((xhr) => {
                        expect(xhr.response.statusCode).to.eq(delete_rk_cost_projections.success);
                    })
                })
            })
        }
        ...
        if (access.view == true && access.delete == false) {
            it('RK Cost Projections delete and Get unauthorized Status', () => {
                //login
                cy.loginByForm(email, password)
                cy.visit('https://frx-wl-one.slashash.dev/fee-rx/rk-cost-projections/all');
                //Verify element on page
                ...
                cy.fixture('ReportRKCostProjections.json').then((data) => {
                    ...
                    cy.wait('@delete_report', { requestTimeout: 10000 });
                    cy.get('@delete_report').then((xhr) => {
                        expect(xhr.response.statusCode).to.eq(delete_rk_cost_projections.unauthorized);
                    })
                })
            })
        }
